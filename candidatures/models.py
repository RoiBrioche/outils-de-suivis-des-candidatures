...
class PeriodeRecherche(models.Model):
    ...
    def clean(self):
        ...
        existing_periods = PeriodeRecherche.objects.exclude(pk=self.pk).filter(date_fin__isnull=False)
        
        for period in existing_periods:
            # Standard overlap logic: StartA <= EndB and EndA >= StartB
            overlap_condition = (
                self.date_debut <= period.date_fin and 
    ...
    def save(self, *args, **kwargs):
        ...
        has_newer_period = PeriodeRecherche.objects.filter(
            date_debut__gt=self.date_debut
        ...
        if self.active:
            active_periods = PeriodeRecherche.objects.filter(active=True).exclude(pk=self.pk)
            for old_period in active_periods:
                ...
                if old_period.date_fin is None and self.date_debut > old_period.date_debut:
...
class Statut(models.Model):
    ...
    code = models.CharField(max_length=50, unique=True, default='', verbose_name="Code technique")
...
class Candidature(models.Model):
    ...
    date_candidature = models.DateField(default=timezone.now, verbose_name="Date de candidature")
    commentaires = models.TextField(blank=True, verbose_name="Commentaires")
    statut_contextuel = models.JSONField(null=True, blank=True, verbose_name="Informations contextuelles du statut")
    ...
    def clean(self):
        if self.periode_recherche:
            ...
            if self.periode_recherche.date_fin and self.date_candidature > self.periode_recherche.date_fin:
                raise ValidationError({
...
class DocumentCandidature(models.Model):
    ...
    chemin_fichier = models.CharField(max_length=500, default='', verbose_name="Chemin du fichier")
    mime_type = models.CharField(max_length=100, blank=True, verbose_name="Type MIME")
    taille = models.PositiveIntegerField(null=True, blank=True, verbose_name="Taille (octets)")
