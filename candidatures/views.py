...
from .forms import (
    ...
    CandidatureForm,
    PisteCandidatureForm,
    StatutForm,
    DocumentCandidatureForm,
    CandidatureSearchForm,
    ContactForm,
)
...
class CandidatureListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["statuts_disponibles"] = Statut.objects.filter(actif=True).order_by(
            "ordre_affichage", "nom"
        )
...
class CandidatureDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        DocumentFormSet = modelformset_factory(
            DocumentCandidature, form=DocumentCandidatureForm, extra=1, can_delete=False
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        formset = DocumentFormSet(
            request.POST, request.FILES, queryset=DocumentCandidature.objects.none()
        )
        ...
        if formset.is_valid():
            with transaction.atomic():
                ...
                messages.success(
                    request, f"{len(instances)} document(s) ajouté(s) avec succès."
                )

        return redirect(
            reverse_lazy(
                "candidatures:candidature-detail", kwargs={"pk": self.object.pk}
            )
        )
...
class CandidatureCreateView(CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.POST:
            context["document_formset"] = DocumentFormSet(self.request.POST)
        else:
...
class PisteCandidatureCreateView(CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if "contact_formset" not in context:
            context["contact_formset"] = ContactFormSet(
                queryset=Contact.objects.none(), prefix="contact"
            )
        return context
    ...
    def form_valid(self, form):
        with transaction.atomic():
            ...
            contact_formset = ContactFormSet(
                self.request.POST, queryset=Contact.objects.none(), prefix="contact"
            )
            ...
            if contact_formset.is_valid():
                ...
                messages.success(
                    self.request, "Piste de candidature créée avec succès."
                )
                return super().form_valid(form)
            else:
    ...
    def form_invalid(self, form):
        messages.error(
            self.request,
            "Erreur lors de la création. Veuillez vérifier les informations.",
        )
...
class PisteCandidatureUpdateView(UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if "contact_formset" not in context:
            context["contact_formset"] = ContactFormSet(
                queryset=self.object.contacts.all(), prefix="contact"
            )
        return context
    ...
    def form_valid(self, form):
        with transaction.atomic():
            ...
            # Instancier le formset avec les données POST et le bon préfixe
            contact_formset = ContactFormSet(
                self.request.POST, queryset=self.object.contacts.all(), prefix="contact"
            )
            ...
            if contact_formset.is_valid():
                ...
                messages.success(
                    self.request, "Piste de candidature mise à jour avec succès."
                )
                return super().form_valid(form)
            else:
                # Si le formset n'est pas valide, réafficher le formulaire avec les erreurs
    ...
    def form_invalid(self, form):
        messages.error(
            self.request,
            "Erreur lors de la mise à jour. Veuillez vérifier les informations.",
        )
        return super().form_invalid(form)
...
class DocumentCandidatureCreateView(CreateView):
    ...
    def get_success_url(self):
        return reverse_lazy(
            "candidatures:candidature-detail", kwargs={"pk": self.object.candidature.pk}
        )
    ...
    def get_object(self, queryset=None):
        ...
        if not pk:
            raise AttributeError(
                "DocumentCandidatureDeleteView must be called with a pk"
            )
        return super().get_object(queryset)
...
def dashboard(request):
    ...
    recent_candidatures = Candidature.objects.select_related("statut").order_by(
        "-created_at"
    )[:5]
...
def contact_remove_from_piste(request):
    ...
    try:
        ...
        return JsonResponse(
            {"success": True, "message": "Contact retiré de la piste avec succès"}
        )
        ...
        with transaction.atomic():
            for contact_data in contacts_data:
                ...
                if not contact_data.get("nom", "").strip():
                    return JsonResponse(
                        {"success": False, "error": "Le nom du contact est obligatoire"}
                    )
...
def candidature_update_field(request, pk):
    ...
    try:
        ...
        if field_name not in allowed_fields:
            return JsonResponse(
                {"success": False, "error": f'Champ "{field_name}" non autorisé'},
                status=400,
            )
        ...
        if field_name == "statut":
            ...
            if field_value:
                try:
                    ...
                    return JsonResponse(
                        {"success": False, "error": "Statut invalide"}, status=400
                    )
            else:
                return JsonResponse(
                    {"success": False, "error": "Le statut ne peut pas être vide"},
                    status=400,
                )
        ...
        if field_name == "statut":
            ...
            response_data["display_value"] = (
                candidature.get_priorite_display() if candidature.priorite else ""
            )
        ...
        return JsonResponse(
            {"success": False, "error": "Format de données JSON invalide"}, status=400
        )
    except Exception as e:
        return JsonResponse(
