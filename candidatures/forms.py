...
class ContactForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            "nom": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "Nom obligatoire"}
            ),
            "prenom": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "Prénom"}
            ),
            "poste_occupe": forms.TextInput(
                attrs={
                    "class": "form-control",
            ...
            "telephone": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "06 12 34 56 78"}
            ),
    ...
    def clean_email(self):
        ...
        if (
            email
            and Contact.objects.filter(email=email)
...
class PisteCandidatureForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if etat == EtatPiste.TRANSFORMEE and not candidature:
            raise ValidationError(
                "Une piste transformée doit être associée à une candidature."
            )
...
class CandidatureForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not self.is_bound:  # seulement si le formulaire n'est pas lié à POST
            self.fields["date_candidature"].initial = (
                self.fields["date_candidature"].initial or timezone.now().date()
            )
...
class DocumentCandidatureForm(forms.ModelForm):
    fichier = forms.FileField(
        label="Fichier", widget=forms.FileInput(attrs={"class": "form-control"})
    )
