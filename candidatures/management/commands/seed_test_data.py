...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            with transaction.atomic():
                ...
                contacts = (
                    self.seed_contacts()
                )  # On récupère les contacts pour les lier après
        ...
        for period in periods:
            ...
            for _ in range(count):
                ...
                if available_contacts:
                    candidature.contacts.add(
                        *random.sample(available_contacts, k=random.randint(0, 2))
                    )
    ...
    def seed_pistes(self, available_contacts):
        ...
        candidatures_by_period = {
            p.id: list(Candidature.objects.filter(periode_recherche=p)) for p in periods
        }
